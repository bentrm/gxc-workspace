...
Ext.define('Deft.mvc.Observer', {
  ...
  statics: {
    ...
    mergeObserve: function(originalParentObserve, originalChildObserve) {
      var childEvent, childEvents, childHandler, childHandlerArray, childObserve, childTarget, convertConfigArray, eventOptionNames, parentEvent, parentEvents, parentHandler, parentHandlerArray, parentObserve, parentTarget, ref, ref1;
      ...
      convertConfigArray = function(observeConfig) {
        var handlerConfig, i, j, len, len1, newObserveEvents, observeEvents, observeTarget, results, thisEventOptionName, thisObserveEvent;
        results = [];
        for (observeTarget in observeConfig) {
          observeEvents = observeConfig[observeTarget];
          if (Ext.isArray(observeEvents)) {
            newObserveEvents = {};
            for (i = 0, len = observeEvents.length; i < len; i++) {
              thisObserveEvent = observeEvents[i];
              if (Ext.Object.getSize(thisObserveEvent) === 1) {
                ...
                for (j = 0, len1 = eventOptionNames.length; j < len1; j++) {
                  thisEventOptionName = eventOptionNames[j];
            ...
            results.push(observeConfig[observeTarget] = newObserveEvents);
          } else {
            results.push(void 0);
          }
        }
      ...
      for (childTarget in childObserve) {
        ...
        for (childEvent in childEvents) {
          ...
          if (!(parentObserve != null ? (ref = parentObserve[childTarget]) != null ? ref[childEvent] : void 0 : void 0)) {
      ...
      for (parentTarget in parentObserve) {
        ...
        for (parentEvent in parentEvents) {
          ...
          if (childObserve != null ? (ref1 = childObserve[parentTarget]) != null ? ref1[parentEvent] : void 0 : void 0) {
  ...
  constructor: function(config) {
    var eventName, events, handler, handlerArray, host, i, len, options, references, scope, target;
    this.listeners = [];
  ...
  isTargetObservable: function(host, target) {
    var hostTarget, hostTargetClass, ref;
    hostTarget = this.locateTarget(host, target);
    if (hostTarget == null) {
      return false;
    }
    if ((hostTarget.isObservable != null) || (((ref = hostTarget.mixins) != null ? ref.observable : void 0) != null)) {
      return true;
    } else {
